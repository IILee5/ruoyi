...
public class RedisDelayEngine<T extends BaseTask>
{
    ...
    public RedisDelayEngine(RedisDelayQueue redisDelayQueue, String queueName, DelayTaskListener<T> taskListener, Class<T> classType)
    {
    ...
    private void errorHandler(T t)
    {
        try
        {
            ...
            {
                log.info("任务重试，消息ID:{}，错误次数:{}", taskId, errorTimes);
                //任务延迟一秒重试,失败次数+1
        ...
        public void run()
        {
            try
            {
                ...
                while (true)
                {
                    try
                    {
                        log.info("延时处理获取队列，队列名称:{}", queueName);
    ...
    {
        // 销毁不会删除已存在队列
        log.info("销毁延时队列，队列名称:{}", queueName);
