package com.ruoyi.framework.api.satoken.at;
...
public class StpUserUtil
{
    ...
    /**
     * 请求头签名校验
     * MD5加密串定长32位，apiKey定长19位，apiSecret定长21位，timestamp定长13位
     * sign = MD5(apiKey + apiSecret + timestamp) + apiKey + timestamp
     * timestamp 为东八区时间(UTC/GMT+08:00)
     */
    public static void checkSign()
    {
        ...
        int apikeyLength = 19;
        int oneMinute = 60000;
        String signStr = ServletUtils.getHeader("sign");
        if (StrUtil.isBlank(signStr))
        {
            throw new NoCheckException("非法密钥");
        }
        // 拆分 sign 请求头
        String sign = StrUtil.subPre(signStr, signLength);
        String apiKeyAndTimestamp = StrUtil.subSuf(signStr, signLength);
        String apiKey = StrUtil.subPre(apiKeyAndTimestamp, apikeyLength);
        String apiSecret = ApiUtils.getApiSecret(apiKey);
        if (StrUtil.isBlank(apiSecret))
        {
            throw new NoCheckException("非法密钥");
        }
        // 只接收一分钟内的请求
        String timestamp = StrUtil.subSuf(apiKeyAndTimestamp, apikeyLength);
        if (NumberUtil.sub(DateUtil.current(), Long.parseLong(timestamp)) > oneMinute)
        {
            throw new NoCheckException("请求已过期");
        }
        // 验证签名
        String serverSign = Md5Utils.hash(apiKey + apiSecret + timestamp);
        if (!StrUtil.equals(serverSign, sign, true))
        {
            throw new NoCheckException("非法密钥");
        }
    }
