        ...
        for (Map.Entry<String, List<RedisListenerMethod>> entry : candidates.entrySet())
        {
            ...
            streamMessageListenerContainer.register(
                    ...
                    message ->
                    {
                        try
                        {
                            logger.info("消息队列 接收消息 : messageId={}, stream={}, body={}", message.getId(), message.getStream(), message.getValue());
                            ...
                            for (RedisListenerMethod rlm : redisListenerMethodList)
                            {
                                ...
                                {
                                    logger.error("消息队列消费出错，错误 : " + e);
                                }
                            }
                            redisTemplate.opsForStream().acknowledge(consumerGroupName, message);
                        } catch (Exception e)
                        {
                            logger.error("消息消费队列注册错误 : " + e);
                        }
                    });
            logger.info("启动消息队列监听器：【{}.{}】，消费者：{}", streamKey, consumerGroupName, consumerName);
        }
    }
